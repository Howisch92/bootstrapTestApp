...
function DigitalDisplay (options)
{
  ...
  var Color = {
    ...
    decimals:	  options.decimals	   || 'Gray'
  };
  ...
  this.drawNumber = function (value, len, decLen, y, height)
  {
	  if (context) {
		this.clear();
	    ...
	    var fixv = Math.floor (value);
	    var decv = Math.round((value - fixv) * (Math.pow(10, decLen)));
	    if (decv >= Math.pow(10, decLen)) {
	    	fixv ++;
	    }
	    ...
	    var off = ((width - (incr * (len+decLen))) / 2.0);
	    if (options.center == false) {
	    	off = 0;
	    }
	
	    // Draw shadow display
	    for (var n = 0; n < len+decLen; n++)
	    {
	    ...
	    if (decLen > 0) {
	    	context.fillStyle = Color.decimals;
	    	for (var n = 0; n < decLen; n++) {
	    		off -= incr;
	    		drawSingleDigit (segs, DigitsSegments[(decv % 10)], off, y);
	    		decv = Math.floor (decv / 10.0);
	    	}
	    	context.font = Math.round(height/2) + 'pt Sans-Serif';
	    	context.fillText (",", off-(incr*0.15), y+(height*0.8));
	    }
